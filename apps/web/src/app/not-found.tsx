...
export default function NotFound() {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4" role="main">
      <div className="w-full max-w-md opacity-0 translate-y-4 transition-all duration-500 ease-out" ref={cardRef}>
        <Card>
          <CardContent className="pt-6 pb-2 flex flex-col items-center text-center space-y-4">
            <div ref={iconContainerRef} className="relative w-24 h-24 mb-2 flex items-center justify-center" aria-hidden="true">
            ...
            <p className="text-muted-foreground font-[&apos;Satoshi&apos;]">We couldn&apos;t find the page you&apos;re looking for.</p>
            ...
            {browserData && (
              <div className="w-full mt-4 text-left text-xs text-muted-foreground border-t pt-4">
                <p className="mb-1"><span className="font-semibold">Requested URL:</span> {browserData.path}</p>
                {browserData.referrer && browserData.referrer !== 'Direct navigation' && (
                  <p className="mb-1"><span className="font-semibold">Referred from:</span> {browserData.referrer}</p>
                )}
                {browserData.previousPages.length > 0 && (
                  <details className="mt-2">
                    <summary className="cursor-pointer font-semibold">Navigation history</summary>
