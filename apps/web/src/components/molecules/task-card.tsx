...
export function TaskCard({ 
  ...
  return (
    <Card className={`relative transition-all hover:shadow-md ${isCompleted ? 'opacity-75' : ''}`}>
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex-1">
              <h3 className={`font-medium text-sm ${isCompleted ? 'line-through text-gray-500' : 'text-gray-900'}`}>
                {task.title}
              </h3>
              {task.description && (
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {task.status === TaskStatus.IN_PROGRESS ? 'In Progress' : 
               ...
               task.status === TaskStatus.COMPLETED ? 'Completed' : task.status}
            </Badge>
            ...
            {task.priority && (
              ...
              >
                {task.priority === TaskPriority.LOW ? 'Low' :
                 ...
                 task.priority === TaskPriority.HIGH ? 'High' : task.priority}
              </Badge>
            )}
          </div>
